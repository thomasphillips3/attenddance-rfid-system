...
- **[P2-2] Raw `prompt()` data-entry flows → modals — parent-facing + leads DONE; other staff remaining.** Parent-facing (donate, makeup) and the **leads add flow** (was 5 chained prompts → one modal form) are now real modals, all verified end-to-end in the browser. ~45 `prompt()` calls remain in other staff tools (recital, donations admin, waivers, skills, company, makeups, pending) — lower priority (staff tolerate rougher UX). `confirm()` guards on deletes are fine to keep.
...
- P1-2 autopay/cards-on-file (biggest parity build — needs Thomas's go-ahead, it's a feature), ~45 remaining staff-side `prompt()` flows → modals, per-page `aria-label`s, P3-4 Subresource Integrity on CDN scripts prompt() flows, P2-3 toast unify, P2-4 aria-labels, P2-5 cron token constant-time check, P2 Square PARTIALLY_PAID semantics, P3s. Full Jackrabbit parity matrix still to expand.
