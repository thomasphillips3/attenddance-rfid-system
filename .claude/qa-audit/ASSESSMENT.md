...
- **[P2-2] Raw `prompt()` data-entry flows → modals — parent + leads + company DONE; ~35 staff prompts remain.** Converted to real modals and verified live: parent-facing (donate, makeup), leads add, and **all three Performance Company flows** (new company / audition / performance — the audition & performance ones previously made staff *type a number* to pick a company from a text list; now a dropdown). Remaining `prompt()` calls are in recital, donations-admin, waivers, skills, makeups, pending — lower priority (staff tolerate rougher UX). `confirm()` guards on deletes are fine to keep.
...
- P1-2 autopay/cards-on-file (biggest parity build — needs Thomas's go-ahead, it's a feature), ~35 remaining staff-side `prompt()` flows → modals, per-page `aria-label`s, P3-4 Subresource Integrity on CDN scripts prompt() flows, P2-3 toast unify, P2-4 aria-labels, P2-5 cron token constant-time check, P2 Square PARTIALLY_PAID semantics, P3s. Full Jackrabbit parity matrix still to expand.
